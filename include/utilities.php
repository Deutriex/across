		...
		class Utilities {
			...
			public static function validaCPF($cpf){
				if (strlen($cpf) == 11){
					...
					$posi = array(10, 9, 8, 7, 6, 5, 4, 3, 2);
					$posi2 = array(11, 10, 9, 8, 7, 6, 5, 4, 3, 2);
					
					for ($i = 0; $i < strlen($CPFx); $i++){
						$sumX += $posi[$i]*intval($CPFx{$i}, 10);
					}
			...
			public static function formatar($string, $tipo = ""){
				...
				if (!$tipo){
					if (strlen($string) == 8)				$tipo = 'cep';
					if (strlen($string) == 10) 				$tipo = 'fone';
					if (strlen($string) == 11){
						if (self::validaCPF($string)){
															$tipo = 'cpf';
						} else {
															$tipo = 'fone';
						}
			...
			public static function numeroParaPreco($num){	
				return "R$ " . number_format($num, 2, ',', '.');
			}

			public static function precoParaNumero(){
			}
